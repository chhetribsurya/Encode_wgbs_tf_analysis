...
output_dir = os.path.expanduser("~/for_chris/batch_I/fimo_motifs_total/motif_meth_analysis/motifs_bin_methplot_total")
...
motif_file_list = glob(join(output_dir, "unique_TFs", "final*"))
...
#df_combined["bin_mid"] = (df_combined["bin_start"] + df_combined["bin_end"])/2
#df_combined["bin_mid"] = df_combined["bin_mid"].astype(int)
...
motif_bin_methplot_df.to_csv(join(output_dir, "unique_TFs", "all_tfs_motif_bin_methplot_heatmap_data.bed"), sep ="\t", header = True, index = True)
